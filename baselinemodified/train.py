...
def train(cfg):
    ...
    for epoch in tqdm(range(cfg.epochs)):
        ...
        for i, batch in enumerate(combined_loader):
            ...
            labelledloss = torch.sum(loss_fn(preds, labels))/ len(labels)

            unlabelledloss = torch.sum(labels.eq(-1).float()* (probabilities>confidence).float() * loss_fn(preds_strong, pseudolabels)) / len(labels)
